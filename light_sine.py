...
if 0:
    ...
    for i in range(n_dist_mat):
        for j in range(n_dist_mat):
            dist_mat[i,j] = math.sin(5*res*((center_dist_mat-i)**2+(center_dist_mat-j)**2)**0.5)#.astype(np.int16)
else:
    dist_mat = pickle.load(open("save.p", "rb"))
    center_dist_mat =np.floor(n_dist_mat/2)
# pickle.dump(dist_mat, open("save_0p01.p", "wb"))
pickle.dump(dist_mat, open("save.p", "wb"))
# print('done')
...
if 0:
    ...
    A_input = np.array([[1, 0], [-1, 0], [0, 1], [0, -1]])
    N = len(A_input[:,1])
